...
if (isset($payatgate_payment_settings['pdf_title'])) {
	$pdf->SetTitle(pdftext($event_name . ' - ' . $payatgate_payment_settings['pdf_title']));
} else {
...
$pdf->AddPage();
//Create the top right of payatgate below header
$pdf->SetFont('Times', '', 12);
$pdf->Cell(180, 0, __('Date: ', 'event_espresso') . date(get_option('date_format')), 0, 1, 'R'); //Set payatgate date
$pdf->Cell(180, 10, __('Primary Attendee ID: ', 'event_espresso') . $attendee_id, 0, 0, 'R'); //Set payatgate number
$pdf->Ln(0);

//Set the top left of payatgate below header
$pdf->SetFont('Times', 'BI', 14);
if (isset($payatgate_payment_settings['payable_to'])) {
	$pdf->MultiCell(0, 10, pdftext($payatgate_payment_settings['payable_to']), 0, 'L'); //Set payable to
} else {
	$pdf->MultiCell(0, 10, pdftext(''), 0, 'L'); //Set payable to
}
$pdf->SetFont('Times', '', 12);
if (isset($payatgate_payment_settings['payment_address'])) {
	$pdf->MultiCell(50, 5, pdftext($payatgate_payment_settings['payment_address']), 0, 'L'); //Set address
} else {
...
$pdf->payatgateTotals(__('Total:', 'event_espresso'), $total_cost, $left, $right);
$text = __('Amount Paid:', 'event_espresso');
$pdf->payatgateTotals($text, $total_amount_pd, $left, $right);
//	$discount = $total_orig_cost - $total_cost;
//	if ($discount > 0) {
//		$text = __('Discount:', 'event_espresso');
//	} else {
//		$text = __('Discount:', 'event_espresso');
//		$pdf->payatgateTotals($text, $discount, $left, $right);
//	}
//}
$total_owing = $total_cost - $total_amount_pd;
$text = __("Total due:", 'event_espresso');
$pdf->payatgateTotals($text, $total_owing, $left, $right);
$pdf->Ln(10);

//Build the payment link and instructions
if (isset($payatgate_payment_settings['pdf_instructions'])) {
	$pdf->MultiCell(100, 5, pdftext($payatgate_payment_settings['pdf_instructions']), 0, 'L'); //Set instructions
} else {
...
$pdf->Output('payatgate_' . $attendee_id . '_' . $event_identifier . '.pdf', 'D');
exit;
