...
class Espresso_PDF extends Espresso_FPDF{
	...
	function Header(){
		...
		//Logo
		if (isset($payatgate_payment_settings['image_url'])&&trim($payatgate_payment_settings['image_url']) !=''){
			$this->Image($payatgate_payment_settings['image_url'],10,8,90);//Set the logo if it is available
		}else{
		...
		if(isset($payatgate_payment_settings['pdf_title']))
			$this->MultiCell(100,10,pdftext($payatgate_payment_settings['pdf_title']),0,'R');//Set the right header
		else
	...
	function payatgateTotals($text,$total_cost,$left_cell = 125, $right_cell = 35){
		global $org_options, $currency_sign;
